"""
Phase 5 統合パーサー v9最終改訂版（修正4）: プロダクション準備完了

v9最終改訂版（修正4）の改善内容:
【修正4の対応】
  1. 「元年」の置換を元号直後のみに限定（安全性向上）
  2. NUMBERED_LISTのlegal_basisにフォールバックを追加
  3. safe_extract_amountで円記号を任意に（円なし金額に対応）

【修正3の対応】
  1. 「元」の過剰置換を修正（「元年」に限定）
  2. extract_comprehensive_law_infoにnormalized_textを渡すよう統一
  3. legal_basisが未設定の場合、legal_basis_normalizedを代入

【修正2の対応】
  1. 元号のハードコード問題を修正（昭和・平成対応）
  2. TB → FB に名称修正（Financing Bills）
  3. 政府短期証券（FB）の発行根拠法令を追加
  4. FBの推定漏れを修正（「第○回」の前にチェック）
...
def normalize_text(text: str) -> str:
    """
    テキストを標準形式に正規化（修正3: 「元年」のみ置換）
    
    修正3の改善点:
    - 「元」を無条件に置換せず、「元年」のみを「1年」に置換
    - これにより「元利金」「元本」が誤変換されない
    """
    ...
    # 修正4: 元号直後の「元年」のみを「1年」に置換
    text = re.sub(r'(令和|平成|昭和)元年', r'\g<1>1年', text)
...
def safe_extract_amount(text: str, field_name: str = "額面金額") -> Optional[int]:
    """
    特定フィールド名の後の金額を抽出（修正4: 円記号を任意に）
    
    修正4の改善点:
    - フィールド名がある場合のみ円記号を任意に
    - 「総額 1,000,000」のようなケースに対応
    """
    # 修正4: 円記号を任意に（円?）
...
BOND_TYPE_MAPPING = {
    ...
    # 政府短期証券（FB）
    '財政法第7条第1項': {
        'category': '政府短期証券（財務省証券）',
        'mof_category': '政府短期証券',
        'description': '財務省証券'
    },
    '財政融資資金法第9条第1項': {
        'category': '政府短期証券（財政融資資金証券）',
        'mof_category': '政府短期証券',
        'description': '財政融資資金証券'
    },
    '特別会計に関する法律第83条第1項': {
        'category': '政府短期証券（外国為替資金証券）',
        'mof_category': '政府短期証券',
        'description': '外国為替資金証券'
    },
    '特別会計に関する法律第94条第2項': {
        'category': '政府短期証券（石油証券）',
        'mof_category': '政府短期証券',
        'description': '石油証券'
    },
    '特別会計に関する法律第94条第4項': {
        'category': '政府短期証券（原子力損害賠償支援証券）',
        'mof_category': '政府短期証券',
        'description': '原子力損害賠償支援証券'
    },
    '特別会計に関する法律第95条第1項': {
        'category': '政府短期証券（石油証券・原子力損害賠償支援証券）',
        'mof_category': '政府短期証券',
        'description': '石油証券・原子力損害賠償支援証券'
    },
    '特別会計に関する法律第136条第1項': {
        'category': '政府短期証券（食糧証券）',
        'mof_category': '政府短期証券',
        'description': '食糧証券'
    },
    '特別会計に関する法律第137条第1項': {
...
def normalize_law_key(law_ref: str) -> str:
    ...
    # 財政融資資金法
    match = re.search(r'財政融資資金法第(\d+)条(?:第(\d+)項)?', law_ref)
    if match:
        article = match.group(1)
        paragraph = match.group(2) if match.group(2) else '1'
        return f'財政融資資金法第{article}条第{paragraph}項'
...
def classify_bond_type(law_key: str) -> Dict[str, str]:
    ...
    if '財政法第7条' in law_key:
        result = BOND_TYPE_MAPPING['財政法第7条第1項'].copy()
        result['confidence'] = 'medium'
        return result
    
    if '財政融資資金法' in law_key:
        result = BOND_TYPE_MAPPING['財政融資資金法第9条第1項'].copy()
        result['confidence'] = 'medium'
        return result
    ...
    if '特別会計に関する法律' in law_key:
        if '第47条' in law_key:
            ...
            result['confidence'] = 'medium'
            return result
        elif '第83条' in law_key:
            result = BOND_TYPE_MAPPING['特別会計に関する法律第83条第1項'].copy()
            result['confidence'] = 'medium'
            return result
        elif '第94条' in law_key:
            if '第4項' in law_key:
                result = BOND_TYPE_MAPPING['特別会計に関する法律第94条第4項'].copy()
            else:
                result = BOND_TYPE_MAPPING['特別会計に関する法律第94条第2項'].copy()
            result['confidence'] = 'medium'
            return result
        elif '第95条' in law_key:
            result = BOND_TYPE_MAPPING['特別会計に関する法律第95条第1項'].copy()
            result['confidence'] = 'medium'
            return result
        elif '第136条' in law_key or '第137条' in law_key:
            result = BOND_TYPE_MAPPING['特別会計に関する法律第136条第1項'].copy()
            result['confidence'] = 'medium'
            return result
...
class TableParserV4:
    ...
    def _parse_item6(self) -> Optional[Dict[str, Any]]:
        ...
        if match:
            ...
            maturity_era = match.group(3)
            maturity_year = match.group(4)
            maturity_month = match.group(5)
            maturity_day = match.group(6)
            total_amount_str = match.group(7)
            ...
            if maturity_year:
                result['maturity_date_text'] = f'{maturity_era}{maturity_year}年{maturity_month}月{maturity_day}日'
    ...
    def _parse_attached_table(self) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(row_pattern, table_text):
            ...
            entries.append({
                ...
                'maturity_date_text': f'{era}{year}年{month}月{day}日',
            })
...
class FBParser:
    """FB（政府短期証券: Financing Bills）パーサー"""
...
# =============================================================================
# 統合パーサー（修正3: normalized_text統一、legal_basis補完）
# =============================================================================
...
class UniversalAnnouncementParser:
    ...
    def parse_announcement(self, file_path: str, raw_record: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], str]:
        """
        告示ファイルをパースし、発行情報を抽出（修正3対応）
        
        修正3の改善点:
        - extract_comprehensive_law_infoにnormalized_textを渡す
        - legal_basisが未設定の場合、legal_basis_normalizedを代入
        """
        ...
        if pattern == 'NUMBERED_LIST':
            ...
            for entry in entries:
                ...
                issuance = {
                    ...
                    # 修正4: entry['law_name']が取れなかった場合はlaw_info['law_key']でフォールバック
                    'legal_basis': (entry.get('law_name') or law_info['law_key']),
            ...
            for entry in entries:
                ...
                issuance = {
                    ...
                    # 修正3: legal_basisが未設定ならlegal_basis_normalizedを代入
                    'legal_basis': law_info['law_key'],
                    'legal_basis_extracted': law_info['law_reference'],
                    'legal_basis_normalized': law_info['law_key'],
                    'legal_basis_source': law_info['source'],
                    'bond_category': law_info['bond_type']['category'] if law_info['bond_type'] else None,
            ...
            parser = FBParser(normalized_text)
            entries = parser.parse()
...
def main():
    """メイン処理"""
    print("=" * 80)
    print("Phase 5 統合パーサー v9最終改訂版（修正4）")
    print("=" * 80)
    print()
    
    # プロジェクト情報（jgb2023環境用に設定済み）
    PROJECT_ID = 'jgb2023'
    DATASET_ID = '20251029'
    ...
    print("v9最終改訂版（修正4）の改善内容:")
    print()
    print("【修正4の対応】")
    print("  1. 「元年」の置換を元号直後のみに限定")
    print("  2. NUMBERED_LISTのlegal_basisにフォールバックを追加")
    print("  3. safe_extract_amountで円記号を任意に")
    print()
    print("【修正3の対応】")
    print("  1-3. 全て対応済み")
    print()
