"""
Phase 5 統合パーサー v9最終改訂版（修正2）: 元号・FB対応
...
def infer_law_from_bond_name(bond_name: str) -> Optional[str]:
    """
    銘柄名から法令を推定（修正2: FBを優先チェック）
    
    修正2の改善点:
    - 政府短期証券を「第○回」より先にチェック
    - これにより「第123回政府短期証券」が建設国債と誤分類されない
    """
...
class TableParserV4:
    ...
    修正2の改善点:
    - _parse_item6と_parse_attached_tableで元号を保持
    - 昭和・平成の告示データでも正確に日付を記録
    """
...
class FBParser:
    """
    FB（政府短期証券: Financing Bills）パーサー（修正2: TB→FB名称修正）
    
    修正2の改善点:
    - TB（Treasury Bills）からFB（Financing Bills）に名称を修正
    - 政府短期証券の正式な略称に統一
    """
...
class UniversalAnnouncementParser:
    ...
    def insert_to_bigquery_layer2(self, announcement_id: str, issuances: List[Dict[str, Any]]) -> Tuple[bool, Optional[str]]:
        """Layer2テーブルへの投入（DELETE→insert方式）"""
        if not issuances:
...
def main():
    """メイン処理"""
    print("=" * 80)
    print("Phase 5 統合パーサー v9最終改訂版（修正2）")
    print("=" * 80)
    print()
    ...
    print("v9最終改訂版（修正2）の改善内容:")
    print()
    print("【修正2の対応】")
    print("  1. 元号のハードコード問題を修正")
    print("  2. TB → FB に名称修正")
    print("  3. 政府短期証券（FB）の発行根拠法令を追加")
    print("  4. FBの推定漏れを修正")
    print()
    print("【最終レビュー対応】")
    print("  1-8. 全て対応済み")
